...
RSpec.describe User, type: :model do
  describe '.from_google' do
    let(:auth) do
      OmniAuth::AuthHash.new(provider: 'google_oauth2', uid: '1234567890',
    ...
    it "creates a new user if one doesn't exist" do
      expect { User.from_google(email: auth.info.email, full_name: auth.info.name, uid: auth.uid) }.to change {
                                                                                                         User.count
                                                                                                       }.by(1)
    end
    ...
    it 'returns an existing user if one exists' do
      user = User.create!(fname: 'Test', lname: 'User', email: auth.info.email, role: 'Admin', password: '123456',
                          uid: auth.uid)
    ...
    context 'with non-allowed email domains' do
      let(:yahoo_auth) do
        OmniAuth::AuthHash.new(provider: 'google_oauth2', uid: '9876543210',
                               info: { email: 'test@yahoo.com', name: 'Test Yahoo' })
      end
      ...
      it "doesn't create a user with yahoo.com email" do
        expect do
          User.from_google(email: yahoo_auth.info.email, full_name: yahoo_auth.info.name, uid: yahoo_auth.uid)
    ...
    context 'with incomplete full_name' do
      let(:first_name_auth) do
        OmniAuth::AuthHash.new(provider: 'google_oauth2', uid: '5555555555',
                               info: { email: 'firstonly@gmail.com', name: 'Firstonly' })
      end
      ...
      it 'handles users with only first name' do
        expect do
          User.from_google(email: first_name_auth.info.email, full_name: first_name_auth.info.name,
                           uid: first_name_auth.uid)
        end.to change {
                 User.count
               }.by(1)
      end
